#!/bin/zsh
find . -type f -not -path "./debian/*" -not -path "./.git*" -not -path ./README.md -not -path ./.pc | sed 's/^.\///g' | awk 'BEGIN {print "#!/usr/bin/env dh-exec"}; {print $1 " => debian/craftos-pc-data/usr/share/craftos/" $1}' > debian/install
