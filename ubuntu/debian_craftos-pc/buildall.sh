#!/bin/zsh
RELEASES=(focal jammy lunar mantic)
...
echo Building and uploading CraftOS-PC v`echo $VERSION | sed "s/${RELEASES[1]}//"`...
for i in $(seq 1 $((${#RELEASES[@]}))); do
    ...
    debuild -S -d
    dput -U ppa:jackmacwindows/ppa ../craftos-pc_${VERSION}-${RELEASES[$i]}${DEBVER}_source.changes
done
